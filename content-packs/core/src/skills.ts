...
const ROWS: readonly SkillRow[] = Object.freeze([
  ...
  ['lumen-foliomoth', '页翼抚慰', 'Page Comfort', '四连智算会修复队伍共享运行值。', 'A 4+ Compute match repairs shared squad runtime.', '页幕', 'Page Veil', '修复共享运行值并提供防护层。', 'Repairs shared runtime and adds a guard layer.'],
  ...
  ['aegis-veribud', '校验露珠', 'Verify Dew', '每阶段首次防护消除修复共享运行值。', 'The first Guard match each stage repairs shared runtime.', '完整校验', 'Full Verify', '净化并修复队伍共享运行值。', 'Cleanses effects and repairs shared squad runtime.'],
  ['aegis-loop-tortoise', '环路甲壳', 'Loop Shell', '进入行动位时为队伍提供防护层。', 'Entering the active slot adds a squad guard layer.', '环路守护', 'Loop Guard', '为队伍共享运行值提供防护层。', 'Guards the shared squad runtime pool.'],
  ['aegis-anchorbee', '特殊锚点', 'Special Anchor', '触发特殊色块时为队伍提供防护层。', 'Triggering a special tile adds a squad guard layer.', '定锚力场', 'Anchor Field', '延迟敌方并保护棋盘一阶段。', 'Delays the enemy and protects the board for one stage.'],
  ...
  ['aegis-dawnguard', '曙光保全', 'Dawn Safeguard', '每场战斗首次共享运行值归零时会被保全。', 'Prevents the first shared-runtime knockout each battle.', '曙光重启', 'Dawn Restart', '修复并保护队伍共享运行值。', 'Repairs and shields shared squad runtime.'],
  ...
  ['glitch-lagtoad', '超时缓冲', 'Timeout Buffer', '共享运行值首次低于一半时延迟敌方。', 'Delays the enemy the first time shared runtime drops below half.', '延迟回击', 'Delayed Payback', '造成伤害并延迟敌方。', 'Deals damage and delays the enemy.'],
  ['glitch-crashfox', '红线爆发', 'Redline Burst', '共享运行值较低时显著提高伤害。', 'Deals substantially more damage at low shared runtime.', '崩溃闪焰', 'Crash Flare', '造成高额伤害并损失少量共享运行值。', 'Deals heavy damage at a small shared-runtime cost.'],
