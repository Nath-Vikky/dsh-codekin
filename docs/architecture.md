...
Codekin `0.3.6-alpha.1` is composed from a headless game engine and validated content packs. The default DSH plugin binds the official core pack, while the same composition API can combine additional packs before the host starts.
...
| `content-packs/core` | The 25 creatures, localized text, assets, encounters, starters, tower rotation, and declarative mechanics shipped in `0.3.6-alpha.1` | Host logic and arbitrary JavaScript mechanics |
...
`pnpm check` runs workspace type checks, unit and integration tests, content-pack linting, a fixed deterministic replay, the combat simulation matrix, production builds, and performance and size budgets. CI additionally installs the packed plugin into an isolated DSH Web profile, exercises the browser and keyboard flow, restarts the host, removes and reinstalls the plugin, and verifies that progress survives every transition. `npm pack --dry-run` can be used to inspect the installable artifact contents.
