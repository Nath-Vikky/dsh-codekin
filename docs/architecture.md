...
Codekin `0.3.2` is composed from a headless game engine and validated content packs. The default DSH plugin binds the official core pack, while the same composition API can combine additional packs before the host starts.
...
| `content-packs/core` | The 25 creatures, localized text, assets, encounters, starters, tower rotation, and declarative mechanics shipped in `0.3.2` | Host logic and arbitrary JavaScript mechanics |
