...
const CODEKIN_ENGINE_VERSION = "0.3.7-rc.1";
...
function createEngineContent(registry) {
	...
	return Object.freeze({
		...
		hasUltimateAppearance: (creatureId) => {
			const id = registry.resolveId(creatureId);
			return creatureMap.get(id)?.rarity === "apex" && registry.asset(`creature:${id}:ultimate`) !== void 0;
		},
...
const CREATURE_ULTIMATE_LEVEL = 60;
/** Appearance never participates in combat stats, growth, or rewards. */
function resolveCreatureAppearance(creature, ultimateAvailable = false) {
	if (creature.level < 30 || creature.appearance === "original") return "original";
	if (ultimateAvailable && creature.level >= 60 && creature.appearance !== "evolved") return "ultimate";
	return "evolved";
}
//#endregion
...
function restoreTraceWildState$1(value, now = Date.now()) {
	...
	for (const raw of rawCreatures) {
		...
		const ultimateUnlocked = level >= 60 && currentEngineContent().hasUltimateAppearance(creatureId);
		const appearance = ultimateUnlocked && (row.ultimateAppearanceUnlocked !== true || savedLevel < 60) ? "ultimate" : savedLevel < 30 && level >= 30 ? "evolved" : row.appearance === "original" || row.appearance === "evolved" && level >= 30 || row.appearance === "ultimate" && ultimateUnlocked ? row.appearance : void 0;
		next.creatures.push({
			instanceId,
			creatureId,
			quality,
...
function applyTraceWildAction$1(current, action, random, now = Date.now()) {
	...
	if (action.type === "set-creature-appearance") {
		...
		if (creature === void 0 || ![
			...
			"evolved",
			"ultimate"
		].includes(action.appearance) || action.appearance === "evolved" && creature.level < 30 || action.appearance === "ultimate" && (creature.level < 60 || !currentEngineContent().hasUltimateAppearance(creature.creatureId))) throw new TraceWildRuleError("invalid-action");
		const next = structuredClone(current);
		const selected = next.creatures.find((row) => row.instanceId === action.creatureInstanceId);
		selected.appearance = action.appearance;
		if (selected.level >= 60 && currentEngineContent().hasUltimateAppearance(selected.creatureId)) selected.ultimateAppearanceUnlocked = true;
		return { state: commit(next, now) };
	}
	...
	switch (action.type) {
		...
		case "feed-material": {
			...
			if (previousLevel < 60 && creature.level >= 60 && currentEngineContent().hasUltimateAppearance(creature.creatureId)) {
...
//#endregion
export { MAX_BOSS_SWAPS_PER_PHASE as $, areAdjacentTiles as A, createEngineContent as At, resolveForcedTiles as B, emptyTowerMaterialReward as C, wildLevelForRoster as Ct, MATCH_BOARD_CELLS as D, CODEKIN_ENGINE_VERSION as Dt, towerSkillTierForFloor as E, xpToNextLevel as Et, findFirstLegalBattleSwap as F, MechanicsContractError as Ft, CAPTURE_HEALTH_RATIO as G, BASE_BOSS_ACTIONS as H, hasBattleMatches as I, assertMechanicsContract as It, MATERIAL_XP as J, CORE_CAPTURE_POWER as K, reshuffleBattleBoard as L, mechanicsContractIssues as Lt, convertRandomBattleTiles as M, withEngineContent as Mt, createMatchBoard as N, CODEKIN_MECHANIC_CONTRACTS as Nt, MATCH_BOARD_SIZE as O, EngineContentError as Ot, findBestBattleSwap as P, CODEKIN_MECHANIC_OPCODES as Pt, MAX_BOSS_BONUS_ACTIONS as Q, resolveBattleSwap as R, MAX_TOWER_FLOOR as S, wildLevelFor as St, towerQualityForFloor as T, wildStats as Tt, BOSS_SKILL_ENERGY_COST as U, BASE_ACTIONS_PER_CREATURE as V, BOSS_SKILL_ENERGY_LIMIT as W, MAX_BONUS_ACTIONS_PER_STAGE as X, MAX_ACTIONS_PER_CREATURE as Y, MAX_BOSS_ACTIONS as Z, ECOLOGY_ADVANTAGE as _, playerStats as _t, CORE_ENGINE_CONTENT as a, PLAYER_QUALITY_MULTIPLIERS as at, CREATURE_ULTIMATE_LEVEL as b, threatPoints as bt, captureChanceForBattle as c, XP_QUALITY_MULTIPLIERS as ct, restoreTraceWildState as d, coreQualityWeights as dt, MAX_CAPTURE_ATTEMPTS as et, settleTraceWildIdleRewards as f, effectivePartyLevel as ft, createCodekinRuntime as g, playerLevelFactor as gt, createCodekinComposition as h, levelForXp as ht, CORE_CONTENT_VIEW as i, PLAYER_QUALITY_GROWTH_BONUSES as it, chooseBossBattleSwap as j, currentEngineContent as jt, MAX_MATCH_CASCADES as k, QUALITY_SKILL_MULTIPLIERS as kt, createInitialTraceWildState as l, activeMinuteBand as lt, normalizeTraceWildAction as m, idleRewardTier as mt, CORE_CODEKIN_RUNTIME as n, MAX_PLAYER_LEVEL as nt, applyTraceSignal as o, QUALITY_ORDER as ot, towerFloorProfile as p, encounterLifetimeMs as pt, MATERIAL_DROP_WEIGHTS as q, CORE_CONTENT_REGISTRY as r, PLAYER_QUALITY_BASE_MULTIPLIERS as rt, applyTraceWildAction as s, WILD_QUALITY_RESISTANCE as st, CORE_CODEKIN_COMPOSITION as t, MAX_MAP_ENCOUNTERS as tt, expireTraceWildEncounters as u, captureChance as ut, TraceWildRuleError as v, qualityIndex as vt, towerBossStats as w, wildQualityWeights as wt, resolveCreatureAppearance as x, totalXpForLevel as xt, CREATURE_EVOLUTION_LEVEL as y, sessionLevel as yt, resolveExistingBattleMatches as z };

//# sourceMappingURL=core-runtime-DfVjJjTV.js.map