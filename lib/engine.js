...
//#endregion
//#region lib/types/packages/engine/src/skills.js
...
function damageForStep(battle, member, counts, chain) {
	...
	let combo = Math.min(2.25, 1 + .25 * (chain - 1) + (hasForktail ? .04 * (chain - 1) : 0));
	...
	for (const member of livingMembers(battle)) {
		...
		switch (member.creatureId) {
			...
			case "lumen-foliomoth":
				if (counts.lumen >= 4) queuePartyHealing(battle, member.maxHp * .03 * scale);
				break;
			case "lumen-lensel":
			...
			case "aegis-veribud":
				if (counts.aegis > 0 && member.passiveStage !== battle.stage) {
					queuePartyHealing(battle, active.maxHp * .02 * scale);
					member.passiveStage = battle.stage;
				}
				break;
			case "aegis-anchorbee":
				if (specialCount > 0) queuePartyShielding(battle, active.maxHp * .04 * scale);
				break;
			case "aegis-steady-ram": {
				const wild = creatureById(battleEncounterCreatureId(battle));
				if (counts[ECOLOGY_ADVANTAGE[wild.ecology]] > 0) queuePartyShielding(battle, stepDamage * .2 * scale);
				break;
			}
...
function applyStageEntryPassives(battle) {
	...
	if (member.creatureId === "aegis-loop-tortoise") queuePartyShielding(battle, member.maxHp * .08 * scale);
	appendBattleLog(battle, {
		turn: battle.turn,
		kind: "switch",
...
function castActiveSkill(state, creatureInstanceId, random) {
	...
	switch (member.creatureId) {
		case "lumen-indeximp":
			...
			break;
		case "lumen-foliomoth":
			queuePartyHealing(battle, battle.partyMaxHp * .08 * scale);
			queuePartyShielding(battle, member.maxHp * .1 * scale);
			break;
		case "lumen-lensel": {
			const wild = creatureById(battleEncounterCreatureId(battle));
		...
		case "forge-sparkmite":
			...
			break;
		case "forge-rivetclaw":
			queuePartyShielding(battle, member.maxHp * .18 * scale);
		...
		case "relay-mesh-jelly":
			...
			break;
		case "aegis-veribud":
			queuePartyHealing(battle, battle.partyMaxHp * .1 * scale);
			break;
		case "aegis-loop-tortoise":
			queuePartyShielding(battle, battle.partyMaxHp * .2 * scale);
			break;
		case "aegis-anchorbee":
			battle.enemyDelayed = Math.max(1, battle.enemyDelayed);
		...
		case "aegis-dawnguard":
			queuePartyHealing(battle, battle.partyMaxHp * .16 * scale);
			queuePartyShielding(battle, battle.partyMaxHp * .08 * scale);
			break;
		case "glitch-null-nibbler":
			battle.wildShield = 0;
