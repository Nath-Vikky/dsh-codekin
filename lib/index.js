...
function enemyTeamDamageForCharge(battle, wildEcology) {
	if (battle.bossAttackCharge <= 0 || battle.partyHp <= 0) return 0;
	const power = Math.min(1.55, Math.max(.55, .35 + .22 * battle.bossAttackCharge));
	const partyPressure = 1 + .55 * Math.max(0, battle.party.length - 1);
	const roll = battle.wildAttack * power * (battle.bossDamageScale / 1e3) * partyPressure * partyAffinity(battle, wildEcology) * 100 / (100 + partyDefense(battle));
	return Math.max(1, Math.round(Math.min(roll, battle.partyMaxHp * .3, battle.partyHp + battle.partyShield)));
}
...
function restoreBattle(root, state) {
	...
	restored.pendingBossDamage = turnOwner === "boss" ? enemyTeamDamageForCharge(restored, wild.ecology) : 0;
