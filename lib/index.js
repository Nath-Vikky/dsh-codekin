...
function tile(ecology, special = "none", lockedActions = 0) {
	return lockedActions > 0 ? {
		ecology,
		special,
		lockedActions
	} : {
...
function collapseAndFill(board, removed, plans, random) {
	const survivors = board.map((current, index) => {
		...
		return {
			tile: planned === void 0 ? current : tile(current.ecology, planned, current.lockedActions),
			source: index
		};
	});
...
function convertRandomBattleTiles(boardValue, ecology, count, random) {
	const board = cloneBoard(boardValue);
	const candidates = board.map((current, index) => current.special === "none" && (current.lockedActions ?? 0) === 0 && current.ecology !== ecology ? index : -1).filter((index) => index >= 0);
...
function activeMember(battle) {
	...
	if (member === void 0 || member.hp <= 0) throw new TraceWildRuleError("conflict");
	return member;
}
...
function shieldMember(member, amount) {
	const limit = Math.round(member.maxHp * .75);
...
function damageForStep(battle, member, counts, chain) {
	...
	for (const ecology of TRACE_ECOLOGIES) {
		...
		total += stats.attack * (count / 3) * combo * element * playerOffenseLevelFactor(member, battle) * 100 / (100 + battle.wildDefense);
	}
...
function lowestHealthMember(battle) {
		...
		switch (member.creatureId) {
			...
			case "lumen-foliomoth":
				if (counts.lumen >= 4) {
					...
					if (target !== void 0) healMember(target, target.maxHp * .03 * scale);
				}
				break;
			case "lumen-lensel":
			...
			case "aegis-veribud":
				if (counts.aegis > 0 && member.passiveStage !== battle.stage) {
					healMember(active, active.maxHp * .02 * scale);
					member.passiveStage = battle.stage;
				}
				break;
			case "aegis-anchorbee":
				if (specialCount > 0) shieldMember(active, active.maxHp * .04 * scale);
				break;
			case "aegis-steady-ram": {
				const wild = creatureById(battleEncounterCreatureId(battle));
				if (counts[ECOLOGY_ADVANTAGE[wild.ecology]] > 0) shieldMember(active, stepDamage * .2 * scale);
				break;
			}
...
function applyResolution(battle, resolution, random, consumeRepeat) {
	...
	for (const step of resolution.steps) {
		...
		const damage = applyWildDamage(battle, damageForStep(battle, active, step.counts, step.chain));
		totalDamage += damage;
...
function applyStageEntryPassives(battle) {
	...
	if (member.creatureId === "aegis-loop-tortoise") shieldMember(member, member.maxHp * .08 * scale);
	appendBattleLog(battle, {
		turn: battle.turn,
		kind: "switch",
...
function nextLivingIndex(battle) {
	for (let offset = 1; offset <= battle.party.length; offset += 1) {
		...
		if (battle.party[index].hp > 0) return {
			index,
			wrapped: index <= battle.activeIndex
		};
	}
}
function damagePartyMember(member, amountValue) {
...
function maybePreventDefeat(battle, target) {
	...
	target.hp = 1;
	shieldMember(target, target.maxHp * .1 * qualityMultiplier(guardian));
	return true;
}
function maybeDelayForLagtoad(battle) {
	...
	if (lagtoad !== void 0 && battle.party.some((member) => member.hp > 0 && member.hp * 2 < member.maxHp)) {
...
function prepareBossIntent(battle, random) {
	...
	let intent;
	if (!battle.bossSkillArmed) intent = tier >= 2 && roll < .16 + tier * .03 ? "sweep" : "strike";
	else {
		...
		else if (tier >= 2 && roll < .26 + tier * .025) intent = "sweep";
		else if (tier >= 3 && roll < .5 + tier * .02 && (wild.ecology === "lumen" || wild.ecology === "glitch")) intent = "lock";
	}
...
function applyEnemyHit(battle, target, wildEcology, power, maximumHealthRatio, random) {
	const targetDefinition = creatureById(target.creatureId);
	if (targetDefinition === void 0) throw new TraceWildRuleError("conflict");
	const roll = battle.wildAttack * power * (.88 + boundedRandom(random) * .24) * affinity(wildEcology, targetDefinition.ecology) * 100 / (100 + memberStats(target).defense);
	const damage = damagePartyMember(target, Math.min(roll, target.maxHp * maximumHealthRatio));
	if (target.creatureId === "forge-rivetclaw" && damage > 0) target.counterPower = .8 * qualityMultiplier(target);
...
function performBossSettlement(battle, random) {
	...
	const target = intendedTarget !== void 0 && intendedTarget.hp > 0 ? intendedTarget : livingMembers(battle)[0];
	let totalDamage = 0;
	if (battle.enemyIntent === "sweep") {
		for (const member of livingMembers(battle)) totalDamage += applyEnemyHit(battle, member, wild.ecology, finalPower * .62, .25, random);
		appendBattleLog(battle, {
			turn: battle.turn,
			kind: "enemy-sweep",
			amount: totalDamage
		});
	} else if (target !== void 0 && target.hp > 0) {
		const strikePower = battle.enemyIntent === "strike" && battle.bossSkillArmed ? finalPower * 1.15 : finalPower;
		totalDamage = applyEnemyHit(battle, target, wild.ecology, strikePower, .35, random);
		appendBattleLog(battle, {
			turn: battle.turn,
			kind: "enemy",
			amount: totalDamage,
	...
	if (battle.bossSkillArmed) {
		...
		switch (battle.enemyIntent) {
			...
			case "corrupt":
				...
				break;
			case "mark": if (target !== void 0) target.energy = Math.max(0, target.energy - 2);
		}
...
function isCaptureWindowAvailable(battle) {
	return battle.mode === "wild" && battle.wildArmor === 0 && battle.wildHp > 0 && battle.captureAttempts < 3 && battle.wildHp / battle.wildMaxHp <= .5;
}
...
function performBattleSwap(state, from, to, random) {
	...
	return {
		outcome: battle.actionsRemaining === 0 ? completeBattleStage(battle, random) : "none",
		animation
	};
}
...
function skipPlayerStage(battle, random) {
	const active = battle.party[battle.activeIndex];
	if (battle.mode !== "wild" || battle.turnOwner !== "player" || battle.captureWindow || battle.actionsRemaining <= 0 || active === void 0 || active.hp <= 0) throw new TraceWildRuleError("conflict");
...
function castActiveSkill(state, creatureInstanceId, random) {
	...
	switch (member.creatureId) {
		case "lumen-indeximp":
			...
			break;
		case "lumen-foliomoth":
			for (const ally of livingMembers(battle)) healMember(ally, ally.maxHp * .08 * scale);
			shieldMember(member, member.maxHp * .1 * scale);
			break;
		case "lumen-lensel": {
			const wild = creatureById(battleEncounterCreatureId(battle));
		...
		case "forge-sparkmite":
			...
			break;
		case "forge-rivetclaw":
			shieldMember(member, member.maxHp * .18 * scale);
		...
		case "relay-mesh-jelly":
			...
			break;
		case "aegis-veribud":
			for (const ally of livingMembers(battle)) healMember(ally, ally.maxHp * .1 * scale);
			break;
		case "aegis-loop-tortoise":
			for (const ally of livingMembers(battle)) shieldMember(ally, ally.maxHp * .2 * scale);
			break;
		case "aegis-anchorbee":
			battle.enemyDelayed = Math.max(1, battle.enemyDelayed);
		...
		case "aegis-dawnguard":
			for (const ally of livingMembers(battle)) {
				healMember(ally, ally.maxHp * .16 * scale);
				shieldMember(ally, ally.maxHp * .08 * scale);
			}
			break;
		case "glitch-null-nibbler":
			battle.wildShield = 0;
		...
		case "glitch-crashfox":
			...
			member.hp = Math.max(1, member.hp - Math.round(member.hp * .08));
			break;
...
function attemptCapture(state, quality, now, random) {
	const battle = state.battle;
	if (battle === void 0 || battle.mode !== "wild" || !battle.captureWindow || battle.captureAttempts >= 3) throw new TraceWildRuleError("conflict");
...
function restoreBattle(root, state) {
	...
	if (party[activeIndex].hp <= 0 || party.every((member) => member.hp <= 0)) return void 0;
	const id = typeof raw.id === "string" && /^battle_[a-z0-9_]{8,64}$/.test(raw.id) ? raw.id : "";
	if (id === "") return void 0;
	const enemyIntent = raw.enemyIntent;
	if (enemyIntent !== "strike" && enemyIntent !== "guard" && enemyIntent !== "disrupt" && enemyIntent !== "corrupt" && enemyIntent !== "mark" && enemyIntent !== "sweep" && enemyIntent !== "lock" && enemyIntent !== "freeze") return void 0;
	const partyAverageLevel = party.reduce((sum, member) => sum + member.level, 0) / party.length;
	...
	if (enemyTargetScope !== "single" && enemyTargetScope !== "all" && enemyTargetScope !== "self") return void 0;
	...
	return {
		...
		enemyIntent,
		enemyTargetScope,
		...Number.isSafeInteger(raw.enemyTargetIndex) && raw.enemyTargetIndex >= 0 && raw.enemyTargetIndex < party.length ? { enemyTargetIndex: raw.enemyTargetIndex } : {},
...
function restoreTraceWildState(value, now = Date.now()) {
	...
	for (const raw of rawEncounters) {
		...
		next.encounters.push({
			...
			quality,
			level,
			captureAttempts: root.schemaVersion === 3 ? safeInt(row.captureAttempts, 0, 3) : 0,
			spawnedAt,
			expiresAt,
