...
function damageForStep(battle, member, counts, chain) {
	...
	for (const ecology of TRACE_ECOLOGIES) {
		...
		const baseContribution = stats.attack * (count / 3) * combo * element * playerOffenseLevelFactor(member, battle) * playerDefenseFactor(battle.wildDefense);
		...
		if (ecology === activeEcology) {
			if (ecology === "aegis") {
				...
				contribution = stats.attack * (count / 3) * combo * element * playerOffenseLevelFactor(member, battle) * playerDefenseFactor(battle.wildDefense * .4) * 1.03;
				signalEffect = {
					kind: "breach",
					ecology,
					amount: Math.max(1, Math.round(contribution - baseContribution))
...
function applyPlayerSignalEffect(battle, effect) {
	if (effect.kind === "repair") {
		...
		if (amount > 0) appendBattleLog(battle, {
			turn: battle.turn,
			kind: "heal",
			amount
		});
...
function bossPhaseDamageCap(battle) {
	...
	return Math.max(1, Math.round(battle.partyMaxHp * Math.min(.6, .34 + qualityPressure + skillPressure + levelPressure)));
}
function enemyDamageForStep(battle, wildEcology, counts, chain) {
	...
	for (const ecology of TRACE_ECOLOGIES) {
		...
		const baseContribution = battle.wildAttack * tilePower * combo * (battle.bossDamageScale / 1e3) * partyPressure * element * (1400 / (1400 + Math.max(0, defense) * 2.2));
		let contribution = baseContribution;
		if (ecology === wildEcology) {
			if (ecology === "aegis") {
				...
				contribution = battle.wildAttack * tilePower * combo * (battle.bossDamageScale / 1e3) * partyPressure * element * (1400 / (1400 + Math.max(0, defense * .35) * 2.2)) * 1.05;
				signalEffect = {
					kind: "breach",
					ecology,
					amount: Math.max(1, Math.round(contribution - baseContribution))
...
function applyBossSignalEffect(battle, effect) {
	if (effect.kind === "repair") {
		...
		return {
			...effect,
			amount: battle.wildHp - before
		};
	}
	if (effect.kind === "guard") {
		const before = battle.wildShield;
		battle.wildShield = Math.min(Math.round(battle.wildMaxHp * .4), battle.wildShield + effect.amount);
		return {
			...effect,
			amount: battle.wildShield - before
		};
	}
	return effect;
}
