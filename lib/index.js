...
function wildStats(definition, levelValue, quality, partySizeValue, partyAverageLevelValue = levelValue) {
	...
	const hpLevelFactor = 1 + .023 * growth + 1e-4 * growth * growth;
	const attackLevelFactor = 1 + .014 * growth + 45e-6 * growth * growth;
	const defenseLevelFactor = 1 + .01 * growth + 35e-6 * growth * growth;
	const hpGapPressure = 1 + Math.min(.9, levelGap * (.014 + .003 * qualityThreat));
	const attackGapPressure = 1 + Math.min(.45, levelGap * (.007 + .0015 * qualityThreat));
	const defenseGapPressure = 1 + Math.min(.4, levelGap * (.006 + .00125 * qualityThreat));
	return {
...
function playerOffenseLevelFactor(member, battle) {
	...
	if (levelDelta < 0) return Math.max(.3, Math.exp(levelDelta / 20));
	return Math.min(1.12, 1 + levelDelta * .0025);
}
...
function damageForStep(battle, member, counts, chain) {
	...
	let combo = Math.min(2.25, 1 + .22 * (chain - 1) + (hasForktail ? .05 * (chain - 1) : 0));
	...
	for (const ecology of TRACE_ECOLOGIES) {
		...
		const contribution = stats.attack * (count / 3) * combo * element * playerOffenseLevelFactor(member, battle) * playerDefenseFactor(battle.wildDefense);
		total += contribution;
...
function enemyDamageEffectiveness(battle, wildEcology) {
	const multiplier = partyAffinity(battle, wildEcology);
	return multiplier > 1.05 ? "advantage" : multiplier < .95 ? "resisted" : "neutral";
}
function enemyDamageForStep(battle, wildEcology, matchedTiles, chain) {
	if (matchedTiles <= 0 || battle.partyHp <= 0) return 0;
	const combo = Math.min(2.4, 1 + .25 * Math.max(0, chain - 1));
	const tilePower = Math.pow(matchedTiles / 3, .9);
	const partyPressure = .48 + .46 * Math.max(0, battle.party.length - 1);
	const defenseFactor = 95 / (95 + Math.max(0, partyDefense(battle)) * 2.2);
	const roll = battle.wildAttack * tilePower * combo * (battle.bossDamageScale / 1e3) * partyPressure * partyAffinity(battle, wildEcology) * defenseFactor;
...
function continueBattle(battle, random) {
	...
	return {
		outcome: completion.outcome,
		...completion.strike === void 0 ? {} : { animation: {
			kind: "match",
			battleId: battle.id,
			actor: "player",
			frames: [],
			strike: completion.strike
		} }
	};
}
...
function castActiveSkill(state, creatureInstanceId, random) {
	...
	switch (member.creatureId) {
		...
		case "forge-anvilback":
			...
			break;
		case "forge-kiln-colossus": {
			const resolution = resolveForcedTiles(battle.board, selectedIndexes(battle.board, "forge", 64), random);
...
function applyTraceWildAction(current, action, random, now = Date.now()) {
	...
	switch (action.type) {
		...
		case "battle-skip-stage": {
			...
			if (result.strike !== void 0) animation = {
				kind: "match",
				battleId,
				actor: "player",
				frames: [],
...
function restoreBattle(root, state) {
	...
	const restored = {
		...
		pendingBossDamage: 0,
		bossDamageScale: Math.max(880, safeInt(raw.bossDamageScale, 1e3, 1120)),
...
function saveRoute(service, lifecycle, requestRejection) {
	return {
		kind: "exact",
...
var TraceWildService = class {
	...
	clearLocalData() {
		const next = createInitialTraceWildState(this.now());
		next.enabled = false;
