...
function readState(
  runtime: Pick<CodekinRuntime, 'restoreTraceWildState'>,
  filename: string,
  now: number,
): TraceWildState | undefined {
  try {
    if (statSync(filename).size > MAX_STATE_BYTES) return undefined
    return runtime.restoreTraceWildState(JSON.parse(readFileSync(filename, 'utf8')) as unknown, now)
  } catch {
    return undefined
  }
}
